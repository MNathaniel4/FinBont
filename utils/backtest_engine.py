...
#         data: pd.DataFrame,
#         signals: pd.Series,
#         benchmark_weights: np.array = None
#     ) -> Dict:
#         """
...
#         trading_days = (signals.diff() != 0).astype(float)
#         if isinstance(trading_days, pd.Series):
#             transaction_costs = (trading_days * self.commission_fixed + 
#                                abs(returns.mean(axis=1)) * trading_days * self.commission_rate)
#         else:
#             transaction_costs = 0
...
#         bench_metrics = self.calculate_metrics(bench_returns, "Benchmark")
...
#                 'Volatilidad Anual': f"{metrics['annual_volatility']:.2%}",
